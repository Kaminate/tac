...
void TacGraphics::Draw2DBox(
  float width,
  float height,
  v4 color,
  TacTexture* texture )
{
  auto oldVertexCount = ( int )mDefaultVertex2Ds.size();
  v2 offsets[] = {
    v2( 0, 0 ),
    v2( 0, -height ),
    v2( width, -height ),
  ...
  for( int iVert = 0; iVert < 4; ++iVert )
  {
    ...
    defaultVertex2D.mPosition = mPenPos2D + offsets[ iVert ];
    mDefaultVertex2Ds.push_back( defaultVertex2D );
  }
  for( int offset : { 0, 1, 2, 0, 2, 3} )
    mDefaultIndex2Ds.push_back( oldVertexCount + offset );
  TacDrawCall drawCall;
  drawCall.mIndexCount = 6;
  drawCall.mColor = color;
  drawCall.mTexture = texture;
  drawCall.mIsText = false;
  if( !drawCall.mScissorDebugging )
  {
    drawCall.mScissorTest = true;
    drawCall.mScissorRectMinUISpace = mPenPos2D + v2( 0, -height );
    drawCall.mScissorRectMaxUISpace = mPenPos2D + v2( width, 0 );
  }
  mDrawCall2Ds.push_back( drawCall );
}
void TacGraphics::Draw2DText(
  ...
  TacErrors& errors )
{
  ...
  auto oldIndexCount = ( int )mDefaultIndex2Ds.size();
  ...
  for( TacCodepoint codepoint : codepoints )
  {
    ...
    fontStuff->GetCharacter( language, codepoint, &fontAtlasCell, errors );
    TAC_HANDLE_ERROR( errors );
    if( !fontAtlasCell )
      continue;

    const int quad_vertex_count = 4;
    auto startingIndex = ( int )mDefaultVertex2Ds.size();
    for( int iVert : { 0, 1, 2, 0, 2, 3 } )
    {
      mDefaultIndex2Ds.push_back( startingIndex + iVert );
    }
    ...
    for( int i = 0; i < quad_vertex_count; ++i )
    {
      ...
      vert2D.mPosition.x = fontAtlasCellX + glTexCoord.x * scaledBitmapWidth;
      vert2D.mPosition.y = fontAtlasCellY + glTexCoord.y * scaledBitmapHeight;
      vert2D.mGLTexCoord.x = TacLerp( fontAtlasCell->mMinGLTexCoord.x, fontAtlasCell->mMaxGLTexCoord.x, glTexCoord.x );
      vert2D.mGLTexCoord.y = TacLerp( fontAtlasCell->mMinGLTexCoord.y, fontAtlasCell->mMaxGLTexCoord.y, glTexCoord.y );
      mDefaultVertex2Ds.push_back( vert2D );
    }
  ...
  drawCall.mIndexCount = ( int )mDefaultIndex2Ds.size() - oldIndexCount;
  drawCall.mColor = color;
  drawCall.mIsText = true;
  if( !drawCall.mScissorDebugging )
  {
    drawCall.mScissorTest = true;
...
TacDrawCall* TacGraphics::GetLastDrawCall()
{
  return &mDrawCall2Ds.back();
}
