...
void Socket::Send( const String& s, Errors& errors )
{
...
void HTTPRequest::AddString( const String& s )
{
  for( char c:s )
  {
    mBytes.push_back( c );
...
void HTTPRequest::AddLine( const String& s )
{
  AddString( s );
  AddNewline();
}
...
void HTTPRequest::FormatRequestHTTP(
...
void HTTPRequest::FormatRequestWebsocket(
  const String& uri,
  const String& host,
