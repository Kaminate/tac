...
static unsigned int stb_adlefloat(unsigned int adlefloat, unsigned char *buffer, unsigned int buflen)
{
    const unsigned long ADLER_MOD = 65521;
    unsigned long s1 = adlefloat & 0xffff, s2 = adlefloat >> 16;
    ...
    for (;;) {
        ...
        if (i == old_i) {
            if (*i == 0x05 && i[1] == 0xfa) {
                ...
                if (stb_adlefloat(1, output, olen) != (unsigned int) stb__in4(2))
                    return 0;
                return olen;
            } else {
