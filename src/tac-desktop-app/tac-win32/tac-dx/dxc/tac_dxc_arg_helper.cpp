...
namespace Tac::Render
{
  ...
  auto DXCArgHelper::GetArgs() -> LPCWSTR*                         { return mArgs->GetArguments(); }
  auto DXCArgHelper::GetArgCount() -> UINT32                     { return mArgs->GetCount(); }
  ...
  {
    const UTF8Path fsPath{ s };
    const UTF8Path ext{ fsPath.extension() };
  ...
  void DXCArgHelper::SaveDebug( const UTF8Path& pdbDir )
  {
    TAC_ASSERT( pdbDir.Exists() );
    dynmc String dir{ pdbDir };
    if( !dir.ends_with( "/" ) ||
        !dir.ends_with( "\\" ) )
