...
namespace Tac
{
  namespace Render
  {
    ...
    static String PreprocessShaderRegister( const String line )
    {
      const int iReplace = line.find( "TAC_AUTO_REGISTER" );
      ...
      return line.substr( 0, iReplace ) + FrameMemoryPrintf( "register( %c%d ); // Autogenerated register from %s",
                                                             resourceLetter,
                                                             iResource,
                                                             __FUNCTION__ );
    }
    ...
    static String PreprocessShaderFXFramework( const String line )
    {
    ...
    static String PreprocessShaderSourceAux( String shaderSourceCode, Errors& errors );
    static String PreprocessShaderLine( String line, Errors& errors )
    {
      ...
      for( String s : preprocessedLines )
      {
        if( s != line )
        {
          return PreprocessShaderSourceAux( s, errors );
        }
      }
      return line;
    }
    ...
    static String PreprocessShaderSourceAux( String shaderSourceCode, Errors& errors )
    {
      ...
      while( shaderParseData.GetRemainingByteCount() )
        result += PreprocessShaderLine( shaderParseData.EatRestOfLine(), errors ) + "\n";
      return result;
    }

    String PreprocessShaderSource( String shaderSourceCode, Errors& errors )
    {
      sShaderRegisterMap.Clear();
      return PreprocessShaderSourceAux( shaderSourceCode, errors );
    }
