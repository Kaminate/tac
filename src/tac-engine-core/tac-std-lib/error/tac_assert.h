...
namespace Tac
{
  struct AssertFns
  {
    ...
    template< typename T >
    static auto HandleVerify( T&& t, const char* str, StackFrame sf ) { if( !t ) HandleAssert( str, sf ); return t; }
  };
}
...
#define TAC_ASSERT_DEPRECATED           TAC_ASSERT_CRITICAL( "Deprecated!" )
...
//#define TAC_ASSERT_INVALID_DEFAULT( v ) default: TAC_ASSERT_INVALID_CASE( v ); return {} // autoformat not smart enough to tab this
...
#define TAC_VERIFY( x )                 Tac::AssertFns::HandleVerify( x, #x, TAC_STACK_FRAME )
