...
namespace Tac
{
  ...
  auto m3::data() dynmc-> dynmc float* { return &m00; }

  auto m3::data() const -> const float* { return &m00; }

  auto m3::Determinant() const -> float
  {
    return
      m00 * ( m11 * m22 - m12 * m21 ) +
      m01 * ( m12 * m20 - m10 * m22 ) +
  ...
  auto m3::Trace() const -> float
  {
    return m00 + m11 + m22;
  }

  auto m3::GetColumn( int i ) const -> v3
  {
    switch( i )
    {
  ...
  auto m3::Transpose( const m3& m ) -> m3
  {
  ...
  auto m3::FromColumns( const v3& c0, const v3& c1, const v3& c2 ) -> m3
  {
    return {
      c0.x, c1.x, c2.x,
      c0.y, c1.y, c2.y,
  ...
  auto m3::FromRows( const v3& r0, const v3& r1, const v3& r2 ) -> m3
  {
    return{
      r0.x, r0.y, r0.z,
      r1.x, r1.y, r1.z,
  ...
  auto m3::RotRadEuler( const v3& eulerRads ) -> m3
  {
    const m3 result
  ...
  auto m3::RotRadEulerInv( const v3& eulerRads ) -> m3
  {
  ...
  auto m3::RotRadX( float rotRad ) -> m3
  {
    float s { Sin( rotRad ) };
    float c { Cos( rotRad ) };
    return {
  ...
  auto m3::RotRadY( float rotRad ) -> m3
  {
    float s { Sin( rotRad ) };
    float c { Cos( rotRad ) };
    return {
  ...
  auto m3::RotRadZ( float rotRad ) -> m3
  {
    float s { Sin( rotRad ) };
    float c { Cos( rotRad ) };
    return {
  ...
  auto m3::RotRadAngleAxis( const float angleRadians, const v3& unitAxis ) -> m3
  {
  ...
  auto m3::Scale( float x, float y, float z ) -> m3
  {
    return {
      x, 0, 0,
      0, y, 0,
  ...
  auto m3::Scale( const v3& scale ) -> m3
  {
    return {
      scale[ 0 ], 0, 0,
      0, scale[ 1 ], 0,
  ...
  auto m3::Translate( float x, float y ) -> m3
  {
    return {
      1, 0, x,
      0, 1, y,
  ...
  auto m3::Translate( const v2& v ) -> m3
  {
    return {
      1, 0, v.x,
      0, 1, v.y,
  ...
  auto m3::CrossProduct( float x, float y, float z ) -> m3
  {
    return { 0, -z, y,
             z, 0, -x,
            -y, x, 0 };
  ...
  auto m3::CrossProduct( const v3& v ) -> m3
  {
    return CrossProduct(v.x, v.y, v.z );
  }

  auto m3::operator() ( int iRow, int iCol ) dynmc -> dynmc float& { return data()[ 3 * iRow + iCol ]; }

  auto m3::operator()( int iRow, int iCol ) const -> const float& { return data()[ 3 * iRow + iCol ]; }

  auto m3::operator[]( int i ) dynmc -> dynmc float&{ return data()[ i ]; }

  auto m3::operator[]( int i ) const -> const float&{ return data()[ i ]; }
...
auto Tac::operator* (float f, const m3& m) -> m3
{
  m3 result { m };
  result *=f;
  return result;
...
auto Tac::operator * ( const m3& m, const v3& v ) -> v3
{
  v3 result;
  for( int r {}; r < 3; ++r )
  {
...
auto Tac::operator * ( const m3& lhs, const m3& rhs ) -> m3
{
  m3 result;
  for( int r {}; r < 3; ++r )
  {
